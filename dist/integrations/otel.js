...
exports.PradvionSpanExporter = void 0;
...
function getIntAttr(attrs, keys) {
    for (const key of keys) {
        const val = attrs[key];
        if (val != null) {
...
class PradvionSpanExporter {
    pradvionClient;
    constructor(pradvionClient) {
        this.pradvionClient = pradvionClient;
    }
    getClient() {
        if (this.pradvionClient)
            return this.pradvionClient;
        try {
    ...
    _processSpan(span, client) {
        const attrs = span.attributes ?? {};
        const spanKind = attrs['openinference.span.kind'] ?? '';
        const genAiSystem = attrs['gen_ai.system'] ?? '';
        const isLlm = (['LLM', 'CHAIN', 'llm'].includes(spanKind) ||
            Boolean(genAiSystem) ||
            Object.keys(attrs).some(k => k.startsWith('llm.') || k.startsWith('gen_ai.')));
        if (!isLlm)
            return;
        ...
        const latencyMs = startNs && endNs
            ...
            : 0;
        const statusCode = (span.status?.code ?? 0) === 2 ? 500 : 200;
        const ctx = (0, context_1.getEffectiveContext)();
        const customerId = attrs['pradvion.customer_id'] ?? ctx.customerId;
        const feature = attrs['pradvion.feature'] ?? ctx.feature;
        const team = attrs['pradvion.team'] ?? ctx.team;
        const conversationId = attrs['pradvion.conversation_id'] ?? ctx.conversationId;
        const environment = attrs['pradvion.environment'] ?? ctx.environment;
        client.track({
            provider,
            model,
            inputTokens,
...
exports.PradvionSpanExporter = PradvionSpanExporter;
function setupOtel(pradvionClient, batch = true) {
    try {
        const { trace, } = require('@opentelemetry/api');
        const { TracerProvider, BatchSpanProcessor, SimpleSpanProcessor, } = require('@opentelemetry/sdk-trace-base');
        ...
        throw new Error('opentelemetry packages required for OTEL integration.\n' +
