...
function compareCost(options) {
    ...
    for (const [key, [inRate, outRate]] of Object.entries(PRICING)) {
        ...
        if (providers && !providers.includes(provider))
            continue;
        if (models && !models.includes(key))
            continue;
        costs[key] = inRate * inputTokens + outRate * outputTokens;
    }
    return _buildResult(costs, inputTokens, outputTokens);
}
function addCustomPricing(provider, model, inputCostPerToken, outputCostPerToken) {
    PRICING[`${provider}/${model}`] = [
        inputCostPerToken,
        outputCostPerToken,
    ];
}
function _buildResult(costs, inputTokens, outputTokens) {
    ...
    return {
        ...
        formatTable() {
            ...
            for (const [model, cost] of this.sortedByCost()) {
                ...
                lines.push(`  ${model.padEnd(45)} $${cost.toFixed(6)}${marker}`);
            }
            lines.push('─'.repeat(55));
            lines.push(`  Max savings: ${maxSavingsPct.toFixed(1)}% ` +
