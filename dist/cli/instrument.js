...
exports.shouldSkipDir = shouldSkipDir;
exports.findFiles = findFiles;
exports.detectAIUsage = detectAIUsage;
exports.generatePatch = generatePatch;
exports.undoInstrumentation = undoInstrumentation;
...
function shouldSkipFile(filePath) {
    ...
    if (SKIP_FILES.has(name))
        return true;
    if (!JS_EXTENSIONS.has(path.extname(filePath)))
        return true;
    return false;
}
...
function detectAIUsage(content) {
...
function generatePatch(content, detection, isTypeScript) {
    if (detection.alreadyInstrumented)
        return null;
    if (!detection.hasOpenAI && !detection.hasAnthropic)
        return null;
    let patched = content;
    ...
    for (const varName of detection.openAIVars) {
        const pattern = new RegExp(`(${escapeRegex(varName)}\\s*=\\s*)` +
            `(new\\s+(?:Async)?OpenAI\\s*\\([^)]*\\))`, 'g');
        patched = patched.replace(pattern, `$1pradvion.monitor($2)`);
    }
...
function undoInstrumentation(content) {
    if (!content.includes('pradvion'))
        return null;
    let result = content
...
function instrument(options) {
    const { target = '.', dryRun = false, undo = false, quiet = false, } = options;
    ...
    for (const filePath of files) {
        ...
        const isTypeScript = ['.ts', '.mts', '.cts'].includes(path.extname(filePath));
        let patched;
        if (undo) {
...
function showDiff(original, patched, filePath) {
